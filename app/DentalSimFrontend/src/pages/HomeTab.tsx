...
// Steps of the welcome tutorial shown on the home page.
const HOME_TOUR_STEPS: TourStep[] = [
    {
        // === ONBOARDING TUTORIAL ===
        // Welcome step: this selector matches no element on purpose, so the
        // GuidedTour card renders centered as a plain welcome pop-up.
        selector: '[data-tour="welcome"]',
        title: 'Welcome to DentalTrain! 🦷',
        description: (
            <>
                <p>
                    <strong>DentalTrain</strong> is a clinical training app where you practise diagnosing
                    real-world dental cases by talking to <strong>AI-powered virtual patients</strong>.
                </p>
                <p>
                    Interview the patient, run clinical tests, examine images and X-rays, then commit to a
                    diagnosis — just like in a real consultation.
                </p>
                <p>
                    Cases cover both <strong>odontogenic</strong> conditions (originating from the teeth) and{' '}
                    <strong>non-odontogenic</strong> ones (other causes of facial pain), so you'll learn to tell
                    them apart.
                </p>
                <p>
                    The goal is to sharpen your <strong>diagnostic reasoning</strong> in a safe, hands-on way,
                    one case at a time. Let's take a quick tour to get you started!
                </p>
            </>
        ),
    },
    {
        selector: '[data-tour="begin-case"]',
        title: 'Begin Case',
        description:
            "Tap here to start a new training case. You'll be paired with a virtual AI patient to interview, examine and diagnose.",
    },
    {
        selector: '[data-tour="stat-cases"]',
        title: 'Cases',
        description: 'This is how many training cases you have completed so far.',
    },
    {
        selector: '[data-tour="stat-accuracy"]',
        title: 'Accuracy',
        description:
            'Your overall diagnostic accuracy — the percentage of cases you have solved correctly.',
    },
    {
        selector: '[data-tour="stat-badges"]',
        title: 'Badges',
        description: 'Achievements you have unlocked. Keep practicing to earn more!',
    },
];

// Note: whether the user has seen the tutorial is stored on their account
// (Firestore field `has_seen_home_tutorial`), so it follows them across devices.
...
const HomeTab: React.FC<{ tourPreview?: boolean }> = ({ tourPreview }) => {
    ...
    useEffect(() => {
        ...
        // Otherwise auto-start it once, only if the account hasn't seen it yet.
        // `userInfo` comes from /auth/profile, which includes has_seen_home_tutorial.
        if (!tourAutoTriggered.current && userInfo && userInfo.has_seen_home_tutorial === false) {
            tourAutoTriggered.current = true;
            const t = window.setTimeout(() => setShowTour(true), 600);
            return () => window.clearTimeout(t);
        }
    }, [tourPreview, userInfo]);

    // Persist on the account that the home tutorial has been seen.
    const markHomeTutorialSeen = async () => {
        setUserInfo((prev: any) => (prev ? { ...prev, has_seen_home_tutorial: true } : prev));
        try {
            ...
            await fetch(`${API_BASE_URL}/auth/tutorial-seen`, {
                ...
                body: JSON.stringify({ which: 'home' }),
            });
        } catch (err) {
            console.error('Failed to mark home tutorial as seen:', err);
        }
    };
    ...
    const handleStartDiagnosis = async () => {
        ...
        try {
            ...
            // === VALIDATION MODE === remember round/progress for the case screen
            if (data.round != null) {
    ...
    return (
        <IonPage>
            <IonHeader className="ion-no-border">
                <IonToolbar className="dentsim-toolbar">
                    <div className="flex items-center justify-between px-4 py-2">
                        ...
                        {/* Right side: Help button + Streak Badge */}
                        <div className="flex items-center gap-2">
                        <button
                            onClick={() => setShowTour(true)}
                            aria-label="Show tutorial"
                            className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-100 border border-gray-200 text-gray-500"
                        >
            ...
            <IonContent fullscreen className="dentsim-content">
                <div className="px-4 pb-8">
                  ...
                  <span className="text-emerald-100">
                    ...
                    <div className="grid grid-cols-3 gap-3 my-5">
                        {/* CASES CARD */}
                        <div data-tour="stat-cases" className="bg-blue-50 rounded-2xl p-4 text-center">
                        ...
                        {/* ACCURACY CARD */}
                        <div data-tour="stat-accuracy" className="bg-green-50 rounded-2xl p-4 text-center">
                        ...
                        {/* BADGES CARD */}
                        <div data-tour="stat-badges" className="bg-purple-50 rounded-2xl p-4 text-center">
                ...
                {/* Welcome tutorial */}
                <GuidedTour steps={HOME_TOUR_STEPS} isOpen={showTour} onClose={closeTour} />
            </IonContent>
        </IonPage>
    );
};
